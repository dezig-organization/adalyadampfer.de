...
window.addEventListener('scroll', () => {
    ...
    if (currentScroll > 50) {
        ...
        header.style.boxShadow = '0 12px 40px rgba(0, 229, 255, 0.18)';
    } else {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    images.forEach(img => {
        img.addEventListener('error', function () {
            ...
            this.style.background = 'linear-gradient(135deg, rgba(0, 229, 255, 0.16) 0%, rgba(255, 43, 214, 0.16) 100%)';
            this.style.display = 'flex';
            this.style.alignItems = 'center';
            this.style.justifyContent = 'center';
            this.alt = this.alt || 'Produktbild';
        });
    });
});
    ...
    if (thumbnails.length > 0 && mainImage) {
        thumbnails.forEach(thumb => {
            thumb.addEventListener('click', function () {
                ...
                this.style.borderColor = 'rgba(0, 229, 255, 0.82)';
        ...
        if (thumbnails[0]) {
            thumbnails[0].style.borderColor = 'rgba(0, 229, 255, 0.82)';
        }
    }
});
...
if (showMoreBtn && hiddenCards.length > 0) {
    showMoreBtn.addEventListener('click', () => {
        ...
        showMoreBtn.innerHTML = isExpanded ? 'Weniger anzeigen' : 'Mehr anzeigen';
...
if (newsletterForm && subscribeBtn && newsletterInput) {
    subscribeBtn.addEventListener('click', function (e) {
        ...
        // Success message
        alert('Dziękujemy za subskrypcję! Wkrótce otrzymasz od nas wiadomość.');
